...
class GCCXML_Handler(xml.sax.handler.ContentHandler):
    ...
    def Constructor(self, attrs):
        if 'name' not in attrs: import pdb; pdb.set_trace()
        name = attrs["name"]
        return typedesc.Constructor(name)

    def _fixup_Constructor(self, const): pass
    
    def OperatorMethod(self, attrs):
        returns = attrs["returns"]
        name = "return_" + returns + attrs["id"]
        return typedesc.OperatorMethod(name, returns)

    def _fixup_OperatorMethod(self, const): pass
    
    def Destructor(self, attrs):
        name = attrs["name"]
        return typedesc.Destructor(name)

    def _fixup_Destructor(self, const): pass
    ...
    def Argument(self, attrs):
        ...
        if parent is not None:
            parent.add_argument(name, typ) # name?
...
##                print "skip %s = %s" % (name, value)
    ...
    def get_result(self):
        interesting = (typedesc.Typedef, typedesc.Enumeration, typedesc.EnumValue,
                       ...
                       typedesc.Variable, typedesc.Macro, typedesc.Alias, typedesc.Field, typedesc.Destructor)
        ...
        for n, i in self.all.items():
            ...
            #if isinstance(n, int): import pdb; pdb.set_trace()
            i.tag_id = n
